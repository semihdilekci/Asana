...
import { markOverdueTasks, runSlaMonitorPipeline } from '../../worker/src/sla-monitor.pipeline.js';
...
describe('Tasks API (integration)', () => {
  ...
  it('SLA pipeline — sla_due_at geçmiş task isSlaOverdue olur', async () => {
    const prisma = app.get(PrismaService);
    ...
    const r = await markOverdueTasks(prisma);
  ...
  it('runSlaMonitorPipeline — SLA_BREACH için in-app bildirim (idempotent bayrak)', async () => {
    const prisma = app.get(PrismaService);
    const u = await prisma.user.findFirst({ where: { firstName: 'Super', lastName: 'Admin' } });
    if (!u) throw new Error('seed');
    ...
    const t = await prisma.task.create({
      data: {
        ...
        assignmentMode: AssignmentMode.SINGLE,
        status: 'PENDING',
        slaDueAt: new Date(nowMs - 3_600_000),
        isSlaOverdue: false,
        slaBreachSentAt: null,
        createdAt: new Date(nowMs - 100 * 3600 * 1000),
      },
    });
    await prisma.taskAssignment.create({
    ...
    expect(r.breachNotified).toBeGreaterThanOrEqual(1);
    const nRow = await prisma.notification.findFirst({
      where: { userId: u.id, eventType: 'SLA_BREACH', channel: 'IN_APP' },
    ...
    expect(t2?.slaBreachSentAt).not.toBeNull();
  });

  it('runSlaMonitorPipeline — SLA_WARNING (≤%80 kalan pencere)', async () => {
    const prisma = app.get(PrismaService);
