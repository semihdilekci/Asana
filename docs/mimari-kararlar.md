...
> **Versiyon:** 0.8 (Taslak — Ortam bazlı SSO stratejisi: dev Google OIDC, prod Red Hat SSO)
> **Son güncelleme:** 30 Nisan 2026
> **Durum:** 🟢 v0.7 kapsamı korunur. **Ek:** Kimlik sağlayıcısı ortam profiline göre ayrıştı ([A-007], [A-009], ADR 0008): geliştirmede Google OpenID Connect, canlıda Red Hat SSO (Keycloak) hedefi; email+şifre bootstrap/CI/E2E için korunur. Kalan açık maddeler: 4 altyapı detayı + 1 süreç dosyası + MVP sonrası ileri güvenlik iterasyonları.
...
**Karar [A-007]:** Birincil kullanıcı girişi **ortam profiline göre** federated **OpenID Connect** ile yapılır:

- **Geliştirme (dev, lokal, isteğe bağlı erken staging):** **Google OAuth 2.0 / OpenID Connect** (Google Cloud Console’da kayıtlı OAuth istemcisi; Authorization Code + PKCE). Amaç: OIDC akışını, callback güvenliğini ve mevcut platform oturum modeli (access JWT + refresh cookie, bkz. [TS-009]) ile entegrasyonu üretim öncesi doğrulamak.
- **Production (canlı):** Kurumsal **Red Hat SSO (Keycloak)** OIDC (gerekirse SAML federation bridge) — holding standardı; Google IdP yalnızca geliştirme içindir.

**Yardımcı / legacy yol:** **Email + şifre** (`POST /api/v1/auth/login`) seed kullanıcılar, CI/E2E, süperadmin dışı bootstrap ve geçiş dönemi için desteklenmeye devam eder; detaylı akış `07_SECURITY_IMPLEMENTATION` ve ADR 0008.

**Karar [A-008]:** MFA/2FA MVP'de yoktur, sonraki aşamada eklenecektir.

**Karar [A-009]:** Kurumsal tek oturum (SSO) **production hedefi olarak** [A-007] ile kapsamdadır (Red Hat SSO). “MVP dışı SSO” ifadesi kaldırılmıştır: OIDC tabanlı SSO geliştirme ve canlı yol haritasının parçasıdır; SAML/OIDC federation’ın tam genişletilmiş senaryoları (çoklu realm, harici partner IdP) yine roadmap’te değerlendirilir.
...
function userMatchesRole(user, role): boolean {
  ...
  for (const rule of role.rules) {
    for (const conditionSet of rule.conditionSets) {
      // OR
      const allConditionsMatch = conditionSet.conditions.every(
        // AND
...
- Şifre hash: **bcrypt** (cost factor 12) — email+şifre yolu ve süperadmin seed için.
- Token modeli: **Access token (JWT, 15 dk) + Refresh token (opaque, httpOnly cookie, 7 gün, rotation'lı)** — giriş yöntemi (OIDC veya local) fark etmeksizin oturum çıktısı aynı kalır.
- Passport stratejileri: `LocalStrategy` (email+şifre / bootstrap), `JwtStrategy` (API guard); **`OpenIDConnectStrategy` veya Passport-openidconnect eşdeğeri** ile önce **Google (dev)**, canlıda **Keycloak / Red Hat SSO** issuer URL’leri yapılandırılır (aynı kod yolu, farklı env).
...
**Smoke test (agent kendi kontrol eder):** 5. Agent feature'ı çalıştırır — local dev server'da veya test container'da. 6. HTTP endpoint varsa curl / HTTPie ile örnek çağrı yapar. 7. DB etkisi varsa DB'yi sorgular (Prisma Studio veya psql). 8. Dosya yükleme varsa yükler, tarama sonucunu bekler. 9. Sonuçları **açık bir rapor** olarak sunar:
...
**Kullanıcı test süreci — zorunlu:** 10. Agent **kullanıcıya test etmesi için talimat verir** — hangi ekrandan, hangi adımlarla test yapılabilir, beklenen davranış ne. 11. Kullanıcı test etmeden agent **merge / push yapmaz**. Bu kritik kuraldır — kullanıcı explicit "test ettim, çalışıyor" demeden `main` branch'e merge yasaktır. 12. Kullanıcı test sırasında sorun bulursa agent düzeltir, testi tekrar eder, yeniden kullanıcıya verir. 13. Kullanıcı onayından sonra CI pipeline otomatik olarak çalışır [INF-006].
...
| 0.8      | 30 Nisan 2026 | **Kimlik sağlayıcısı ortam stratejisi:** [A-007] güncellendi — geliştirmede **Google OIDC**, production’da **Red Hat SSO (Keycloak)**; email+şifre yardımcı yol. [A-009] SSO’nun production hedefi olarak kapsama alınması. [TS-009] Passport OIDC stratejisi (dev Google / prod Keycloak), oturum modeli değişmez. **ADR 0008** eklendi (`docs/adr/0008-dev-google-oidc-prod-redhat-sso.md`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
