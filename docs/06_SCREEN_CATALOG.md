...
**Kimlik modu (ortam):** Geliştirmede birincil giriş **Google ile oturum aç** (OIDC redirect) olabilir; production’da birincil **Red Hat SSO** olacak şekilde aynı ekran IdP butonu ile genişletilir (`mimari-kararlar.md` [A-007], ADR 0008). Email+şifre formu yardımcı yol olarak kalabilir (seed, E2E, operasyonel bootstrap). OIDC aktifken başlık/açıklama metinleri kurumsal IdP’ye göre uyarlanır; token ve rıza sonrası yönlendirme davranışı aşağıdaki email+şifre akışı ile aynıdır.
...
- **API (list):** `GET /api/v1/notifications?isRead=false|true|all&eventType=...&cursor=...`
- **API (mark-read):** `POST /api/v1/notifications/:id/mark-read`
- **API (mark-all-read):** `POST /api/v1/notifications/mark-all-read`
- **Query key:**
  - `queryKeys.notifications.list({ isRead, eventType, cursor })`
...
- **URL state:** `isRead`, `eventType`, `cursor` query param'da
- **Form state:** Yok

##### Durum Ekranları

- **Loading:** Satır skeleton'ları (5 adet)
- **Empty (okunmamış filter):** EmptyState — Bell ikonu + "Okunmamış bildiriminiz yok" + "Tümünü gör" linki (`?isRead=all`)
...
**Route:** `/admin/email-templates/:eventType/edit`
...
- **API (update):** `PUT /api/v1/admin/email-templates/:eventType` body: `UpdateEmailTemplateSchema` — `{ subjectTemplate, htmlBodyTemplate, textBodyTemplate, requiredVariables }`
- **API (preview):** `POST /api/v1/admin/email-templates/:eventType/preview` body: `{ subjectTemplate, htmlBodyTemplate, textBodyTemplate, variables? }` → render response
