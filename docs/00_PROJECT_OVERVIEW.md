...
- **Kimlik doğrulama ve oturum yönetimi** — ortam profile göre **OpenID Connect (OIDC)** ile birincil giriş: geliştirmede **Google OAuth/OIDC**, canlıda **Red Hat SSO (Keycloak)** hedefi; access/refresh token mimarisi, oturum bütünlüğü koruması aynı kalır. Email + şifre yolu seed, CI/E2E ve bootstrap için korunur; şifre sıfırlama bu yol için geçerlidir (detay: `docs/mimari-kararlar.md` [A-007], ADR 0008).
...
- **Dış sistem entegrasyonları** — SAP MM, SAP HR, PowerBI planlı ancak MVP dışı. **Red Hat SSO** production kimlik hedefi olarak mimari kapsamdadır (MVP öncesi/ile OIDC entegrasyonu; `mimari-kararlar.md` [A-007]).
